import Navigation from '@/components/navigation';
import Footer from '@/components/footer';
import { Link } from 'wouter';
import { CheckCircle, ArrowLeft } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { useSEO } from '@/hooks/use-seo';

export default function ContactThankYou() {
  useSEO({
    title: 'Thank You - Contact Inquiry Submitted',
    description: 'Thank you for your inquiry. Our team will review your request and get back to you within 24 hours with a response.',
  });

  return (
    <div className="min-h-screen bg-background">
      <Navigation />
      
      <section className="min-h-screen flex items-center justify-center py-16 bg-gradient-to-br from-primary to-primary/80">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-white">
          <div className="success-checkmark w-24 h-24 mx-auto mb-8 bg-white rounded-full flex items-center justify-center" data-testid="success-checkmark">
            <CheckCircle className="text-4xl text-primary w-12 h-12" />
          </div>
          
          <h1 className="text-4xl sm:text-5xl font-bold mb-6" data-testid="thank-you-title">Thank You!</h1>
          <p className="text-xl mb-4" data-testid="thank-you-subtitle">Your inquiry has been received.</p>
          <p className="text-lg text-white/90 mb-8" data-testid="thank-you-description">
            Our team will review your request and get back to you within 24 hours with a response.
          </p>
          
          <div className="bg-white/20 backdrop-blur-sm rounded-xl p-6 mb-8" data-testid="next-steps">
            <p className="font-semibold mb-2">What's Next?</p>
            <ul className="text-left space-y-2 text-white/90">
              <li className="flex items-center">
                <CheckCircle className="mr-2 h-5 w-5" />
                Our team will analyze your inquiry
              </li>
              <li className="flex items-center">
                <CheckCircle className="mr-2 h-5 w-5" />
                We'll prepare a detailed response
              </li>
              <li className="flex items-center">
                <CheckCircle className="mr-2 h-5 w-5" />
                You'll hear from us within 24 hours
              </li>
            </ul>
          </div>
          
          <Link href="/">
            <Button className="bg-white text-primary px-8 py-3 rounded-lg font-semibold hover:bg-opacity-90 transition-all" data-testid="back-to-home">
              <ArrowLeft className="mr-2 h-5 w-5" />
              Back to Home
            </Button>
          </Link>
        </div>
      </section>
      
      <Footer />
    </div>
  );
}
